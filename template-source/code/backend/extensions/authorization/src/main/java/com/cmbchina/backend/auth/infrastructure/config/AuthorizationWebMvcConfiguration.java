...
public class AuthorizationWebMvcConfiguration implements WebMvcConfigurer {
    ...
    public AuthorizationWebMvcConfiguration(ResourcePermissionInterceptor interceptor) {
        this.interceptor = interceptor;
    }

    @Override
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(interceptor).order(200).addPathPatterns("/**").excludePathPatterns("/api/login/mock", "/actuator/**");
    }
}
