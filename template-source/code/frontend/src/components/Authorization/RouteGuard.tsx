...
export function RouteGuard({ resourceKeys, children }: PropsWithChildren<{ resourceKeys: readonly string[] }>) {
  const { state } = useAuth();
  const { hasAllPermissions } = usePermission();
  return hasAllPermissions(resourceKeys) ? <>{children}</> : <AuthStateView state={state} />;
}
