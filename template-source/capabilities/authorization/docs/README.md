...
V2 只登记 `001-schema.sql` 为 Schema Asset。Template Service 将其编译为 `ADD_FILE`，由 DevAgent Studio 投放到 `backend/docs/auth/sql/ddl.sql`；`AuthorizationBootstrapCommand` 显式读取该路径。数据库执行不属于 Strategy Executor，也不记录在 TemplateStateV2。
