...
18. 冻结 Generate / Update 语义等价要求：Generate 不再使用第二套高阶 renderer，必须消费同一 Atomic Registry；当前 Release 的 Generate Materializer 输出必须与 XCodeAgent 在 pristine Base 上执行同一 Strategy 集合得到的结果一致。
19. 修正 XCodeAgent mode 选择语义：正常 Template Preparation 固定走 `APPLY`，`RECONCILE` 只允许显式健康修复；不得因 requestedConfig 与当前 State 相同就自动切换为 `RECONCILE`。
...
原因不是“永远不允许维护”，而是当前没有一个已经冻结、可由 XCodeAgent Atomic Executor 精确执行的 AuthProvider 维护规则。V2 不允许为了保留旧设计而继续声明空参数 `TRANSFORM_FILE`。未来如确有维护需求，必须先定义新的明确 Atomic Strategy，再将该 Addition 的 `maintainPolicy` 升级为 `STRATEGY`。
...
strategies:
  ...
  - id: frontend.authorization.register-provider
    ...
    order: 200
    parameters:
      anchor: "// xcodeagent:capability-providers"
      position: before
      managedMarker: "authorization-provider"
      content: |-
        /* xcodeagent:authorization-provider:begin */
        AuthProvider,
...
ENSURE_IMPORT
+
TEXT_ANCHOR_INSERT(anchor = // xcodeagent:capability-providers)
```
...
export const useCapabilityMenus = (menus: Route[]): Route[] => {
  let current = menus;
  // xcodeagent:capability-menu-hooks
  return current;
};
```
...
ENSURE_IMPORT
+
TEXT_ANCHOR_INSERT(anchor = // xcodeagent:capability-menu-hooks)
```

插入内容固定为静态、无条件的顶层 Hook 调用，例如：

```tsx
/* xcodeagent:authorization-menu-transform */
...
XCodeAgent 当前 `NPM_BUILD / NPM_TEST` 会先执行：
...
每个 structural astSelector 用 XCodeAgent 当前 parser 在 Base 上唯一命中
...
即使 requestedConfig 与当前 `TemplateState.requested` 完全相同，正常调用也仍然使用 `APPLY`。是否存在 Release Refresh 必须由 Template Service 根据当前不可复用的 `templateRevision` 判断，而不能由 XCodeAgent 仅根据 requestedConfig 猜测。
...
### 7.4 XCodeAgent Mode 选择规则

当前 XCodeAgent 中“requestedConfig 与 State.requested 相同 → 自动 RECONCILE”的推导必须删除。
...
- 当前 XCodeAgent `ADD_FILE` executor 的执行事实只依赖 `target + payloadRef`；`parameters` 当前固定输出 `{}`，Addition identity 只属于 Service Decision/State，不复制成执行 DSL。
...
V1 migration 声明曾投放
→ backend/src/main/resources/xcodeagent/migrations/authorization/001-schema.sql
```
...
Migration Execution
→ 由 Bootstrap / XCodeAgent 的显式执行协议保证
```
...
若未来需要由 Update Package 动态改变执行时机，必须先扩展 XCodeAgent `StrategyUpdatePackageV2`，再同步修改本方案；不得绕过当前唯一 Wire Contract。
...
```text
/* xcodeagent:<managedMarker>:begin */
... managed content ...
/* xcodeagent:<managedMarker>:end */
```

XCodeAgent Executor 的冻结语义：
...
但当前 XCodeAgent Executor 不消费任何 precondition DSL，因此 Template Service V2 当前统一输出空对象：
...
未来如要启用 precondition，必须先修改 XCodeAgent Executor / Validator 并重新冻结双端 Contract。
...
Service 只生成计划，不执行；XCodeAgent 是唯一 Validation Executor。
  ...
  - id: authorization.postcondition
    ...
    checks:
      ...
      - type: STRUCTURE_CHECK
        ...
        containsAll:
          - xcodeagent:authorization-page-route
...
XCodeAgent 对 `NPM_BUILD / NPM_TEST` 固定先执行：
...
XCodeAgent `TemplateEngineClient` 对非 2xx 不得只保留 HTTP status；应解析上述结构并把 `code` 带入 `TemplateEngineError`，以便上层区分协议冲突和服务故障。
...
→ 写入 .xcodeagent/template-state.json
→ ZIP
```

`GenerateMaterializerV2` 不允许重新解释旧 `point/module/export/...` DSL；它消费的必须是与 Update 相同的 Atomic Registry Entry / 编译结果。

当前 login / authorization 已收口为 `ENSURE_IMPORT + TEXT_ANCHOR_INSERT` 的稳定 Base Surface，因此 Java 端可以实现简单、确定性的 in-memory materialization，而不需要复制 XCodeAgent 的任意 AST Executor。
...
```text
Python XCodeAgent ModificationStrategyExecutorV2
```
...
Update
→ XCodeAgent current Workspace Working Copy
```
...
=
XCodeAgent Update Executor input contract
```
...
→ XCodeAgent Validation Executor
```
...
XCodeAgent 仍会按 requested equality 自动选择 RECONCILE
...
正常 Template Preparation 一律由 XCodeAgent 请求：
...
==
Python XCodeAgent Executor result
```
...
必须能被 XCodeAgent `validate_template_package` 直接接受。
...
XCodeAgent no longer auto-infers RECONCILE from requested equality
...
legacy `backend/src/main/resources/xcodeagent/migrations/authorization/001-schema.sql` target is not silently accepted for a Bootstrap consuming `backend/docs/auth/sql/ddl.sql`
asset deployment does not imply execution
Bootstrap / XCodeAgent only consumes an explicit, parseable execution trigger frozen in the Release / Bootstrap contract
...
XCodeAgent preserves Engine error code
...
P0-7  修改 OpenAPI / EngineController / Error Contract
  ...
  ↓
P0-8  修改 XCodeAgent mode 选择与 TemplateEngineClient error parsing
  ↓
P0-9  打通 Service Contract Test + XCodeAgent Contract Test
  ↓
...
```text
XCodeAgent Wire Contract 本身要新增/修改 Strategy Type
